...
def eval_2015(res_folder,gt_folder ,gt_name = 'MMVText_gt.zip'):
...
#             break
    ...
    gtmitfile = os.path.join(current_folder, gt_name)
#     filenames = os.listdir(gt_folder)
    zip = zipfile.ZipFile(gtmitfile, "w", zipfile.ZIP_DEFLATED)
    for idx,filename in enumerate(filenames):
        filepath = os.path.join(gt_folder, filename.replace("res","gt"))
        zip.write(filepath, filename)
#         if idx>200000:
#             break
    zip.close()
...
def getresult(path,gt_folder):
    # rrc_evaluation_funcs.main_evaluation(None, default_evaluation_params, validate_data, evaluate_method)
    #eval_2015('../../test')
    f_score_new = eval_2015(path,gt_folder)
    return f_score_new

if __name__=='__main__':

    res_folder = "output/GoMPP_DSText/preds_det/"
    gt_folder = "dataset/DSText/Test_det_anno/"
    getresult(res_folder,gt_folder)